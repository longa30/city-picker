...
(function (factory) {
    ...
    CityPicker.prototype = {
        ...
        render: function () {
            var p = this.getPosition(),
                ...
                dropdown = '<div class="city-picker-dropdown" style="left:0px;top:100%;' +
                    ...
                    (this.includeDem('country') ? '<a data-count="country">乡镇</a>' : '') +
                    (this.includeDem('village') ? '<a data-count="village">村</a>' : '') + '</div>' +
        ...
        defineDems: function () {
            var stop = false;
            $.each([PROVINCE, CITY, DISTRICT,COUNTRY,VILLAGE], $.proxy(function (i, type) {
                if (!stop) {
                    this.dems.push(type);
                }
        ...
        output: function (type) {
            ...
            code = (
                type === PROVINCE ? 86 :
                    ...
                    type === COUNTRY ? this.$district && this.$district.find('.active').data('code') :
                    type === VILLAGE ? this.$country && this.$country.find('.active').data('code') :
                    code
            );
            if (typeof this.$element.attr('data-url') !== 'undefined'  && typeof code !== 'undefined' ) {
                var param = {};
                param["id"] = code;
                $.getJSON(this.$element.attr('data-url'),param,function (resData) {
                    var isArr = false;
                    if ($.isPlainObject(resData)) {

                        // alert(isArr + ">>>"+ JSON.stringify(resData));
                        $.each(resData, function (code, address) {
                            isArr =!(address instanceof String);
                            data = isArr ? {} : [];
                            return false;
                        });
                        $.each(resData, function (code, address) {
                            ...
                            if (isArr) {//type === PROVINCE
                                provs = [];
                                for (var codes in address){
                                    if (address[codes] === value) {
                                        matched = {
                                            code: codes,
                                            address: address[codes]
                                        };
                                    }
                                    provs.push({
                                        code: codes,
                                        address: address[codes],
                                        selected: address[codes] === value
                                    });
                                }
                                data[code] = provs;
                                // alert(isArr + ">>>"+ JSON.stringify(data));
                            } else {
                                if (address === value) {
                                    matched = {
                                        code: code,
                        ...
                        var html = isArr ? _this.getProvinceList(data) : _this.getList(data, type);
